...
The following graph, generated by the function "seasonal_decompose()", briefly shows the seasonality within the dataset we were dealing with. Besides, we also manually step through the STR decomposition process on the training data, which allows us to use specific parts while computing the SARIMA model in the next section.

<img src="images/bikeway_str.png?raw=true"/>

### 4. SARIMA model 

To find the proper model, we went through process that included finding an ideal number of differences that make the data more stable, and picking the ideal ARIMA model by taking AIC and BIC into consideration. At the end, we chose (5,3,2) as our final pick of the ARIMA model.

As shown in the graph, the ARIMA model itself predicted trend only. In order to make the prediction more realistic and applicable, we added the seasonality back and now we have a better prediction that has a fluctuation.   
