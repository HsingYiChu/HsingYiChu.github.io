...
The following graph, generated by the function "seasonal_decompose()", briefly shows the seasonality within the dataset we were dealing with. Besides, we also manually step through the STR decomposition process on the training data to use specific parts while computing the SARIMA model in the next section.
